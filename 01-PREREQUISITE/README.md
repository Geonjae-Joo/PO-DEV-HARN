...
다섯 규칙 모두 **여러 레이어·스킬이 공유**하는 foundation 규칙이므로 최상위 `rules/`에 둔다 (단일 스킬 전용 규칙이 없음).
...
| `rules/spine-ids.md` | SCR-/CMP-/REQ-/ENT-/EXT-/NOTE-/NFR-/JRN-/SPEC-/T###/DP- 채번 규칙. 전 레이어 공통 적용. |
| `rules/tech-stack.md` | **앱 스택의 단일 출처.** 백엔드·프론트엔드 스택을 ①에서 프로젝트별로 확정한다(고정값 아님). 현재 선택 예시: 백엔드 Spring Boot / 프론트엔드 React+Vite+TS+Tailwind+shadcn/ui. ②·③·스킬·훅이 이 파일을 따른다. 변경 시 DECISIONS.md 갱신. |
| `rules/ops-stack.md` | **운영 스택의 단일 출처.** 형상관리(GitHub\|GitLab)·CI/CD·배포 타깃(k8s\|Docker\|온프렘)·관측성(Phoenix\|Langfuse) 결정을 프로젝트별로 확정(고정값 아님). SPEC-OPS-000 명세의 *도구 선택*에 해당. ③ Phase 0·γ가 따른다. |
