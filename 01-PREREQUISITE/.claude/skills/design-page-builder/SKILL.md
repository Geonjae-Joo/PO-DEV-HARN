...
inputs:
  - output/foundation/design-system/ds-allowlist.md   # 허용 집합 원본
outputs:
...
`ds-allowlist.md`를 토대로 **빈 페이지 템플릿(design page)** 을 만든다.
...
6. **검증(필수)** — 저장 직후 Bash로 전용 린터를 직접 호출한다:
   ...
   ```bash
   python .claude/skills/design-page-builder/scripts/design-page-lint.py
   ```
...
- DS 허용 집합 밖의 컴포넌트를 **발명하지 않는다** (`.claude/rules/ds-closure.md`).
- 모든 템플릿은 `DP-` 스파인 ID를 가진다 (`.claude/rules/spine-ids.md`).
