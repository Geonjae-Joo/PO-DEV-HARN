...
| `data_source` | 보여주거나 다루는 데이터는 어디서 오는가? (query/mutate/export action의 outcome.target이 ENT-/EXT-) | **error** | 차단 |
...
| `validation` | 입력(form 계열) 컴포넌트의 검증 규칙은 무엇인가? (action.type=validation 또는 note의 필수/형식/범위 언급) | **error** (form 계열) | 차단 |
...
| `navigation_out` | navigate target(SCR-)이 model_repo에 실존하는가? (미래 화면 허용) | warn |
