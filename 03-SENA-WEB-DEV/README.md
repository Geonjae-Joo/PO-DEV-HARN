# ③ SENA-WEB-DEV — 개발 레이어 (SDD + TDD 하네스)
...
2. Freeze   구현 중 화면은 소프트 프리즈. PI 편집은 change-order 큐에 누적.
3. Change Order  PI 재확정 시 스파인 ID 단위 diff + blast radius 계산 → 변경 지시서.
4. 개발자 판정
