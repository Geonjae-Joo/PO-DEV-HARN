...
name: gate-a-check
description: >
  Gate A 종합 판정 스킬. lint error 0 + sufficiency pass + 전 action user_confirmed + PI 승인을
  확인하고 통과 시 status: confirmed로 전환한다.
  PI가 "확정", "confirm", "Gate A", "승인" 을 명시적으로 요청할 때만 실행.
when_to_use: PI가 화면 정의를 확정하고 개발로 넘기겠다고 명시적으로 요청할 때.
...
3. **차단 시** — stderr에 출력된 미충족 조건을 PI에게 제시하고 해소 요청
