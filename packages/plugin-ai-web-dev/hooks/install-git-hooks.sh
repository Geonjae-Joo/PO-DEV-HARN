...
#
# 사용: app_repo 루트(또는 강제할 git 저장소)에서 플러그인 경로의 이 스크립트를 실행
#   bash "$CLAUDE_PLUGIN_ROOT/hooks/install-git-hooks.sh"
# 환경변수:
#   PYTHON=python3            # 파이썬 실행기 지정 (기본: python → 없으면 python3)
#   HARNESS_TEST_CMD="..."    # tdd-gate가 실행할 테스트 명령(foundation/decisions/tech-stack.md 핀). 훅에 기록됨.
# ────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
[ -n "${TEST_CMD_LINE}" ] && echo "  HARNESS_TEST_CMD 고정됨" || \
  echo "  HARNESS_TEST_CMD 미지정 — tdd-gate 자동 탐지 사용(foundation/decisions/tech-stack.md 권장)"
