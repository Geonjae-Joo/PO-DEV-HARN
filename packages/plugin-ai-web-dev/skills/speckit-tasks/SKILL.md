...
- **테스트는 OPTIONAL이 아니다(하니스 override).** 모든 구현 태스크 **앞에** 대응 테스트 태스크를 배치한다(test-first). 테스트 없는 구현 태스크 생성 금지 — `tdd-gate.py`가 commit에서 차단.
- **2계층**: 각 REQ-는 **API 레벨** + **화면 레벨** 테스트. decision table 모든 row·state machine 모든 전이 ≥1 테스트.
...
- **하니스 절차 요약**:
  1. **태스크 채번** — 구현 단위마다 `T###` ID 부여 (스파인 ID, `packages/harness-core/rules/spine-ids.md` 채번 규칙 준수)
