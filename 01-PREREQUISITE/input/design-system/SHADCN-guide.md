...
>
> ⚠️ 이 가이드는 **프론트엔드 스택을 React + shadcn/ui로 정한 경우**의 예시다. 스택은 `rules/tech-stack.md`에서 프로젝트별로 정하며 고정이 아니다. 다른 프레임워크/DS를 쓰면 그에 맞는 준비 절차로 대체하되, "DS를 `design-guide.md` 허용 집합으로 등록한다"는 원칙은 동일하게 유지한다.
>
...
- 이 예시가 전제하는 스택(`rules/tech-stack.md`에서 확정, 고정값 아님): React 18 + Vite + TypeScript + Tailwind CSS + shadcn/ui.
