...
- **진입 조건**: Gate B approve + `/speckit-analyze` CRITICAL=0 + bl-analyst open=0. 미충족 시 실행 금지.
- **태스크 1개씩 TDD 루프**: **RED**(test-author가 실패 테스트 작성 + 실패 증명) → **GREEN**(통과 최소 구현) → **REFACTOR** → **COMMIT**. red 미증명 시 green 진입 금지.
