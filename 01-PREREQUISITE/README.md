...
> **배치 원칙 (②와 동일):** 단일 스킬이 직접 호출하거나 그 스킬 전용인 스크립트·파일은 **해당 스킬 폴더 아래에 공존**시킨다. 저장 이벤트로 자동 실행되는 훅(hooks.json)과 여러 스킬·레이어가 공유하는 규칙은 **`.claude/hooks/`·`.claude/rules/`** 에 둔다.
...
hooks는 `.claude/hooks/hooks.json`에 Claude Code hook 이벤트로 정의된다. 스크립트 경로: `${CLAUDE_PROJECT_DIR}/.claude/hooks/`
...
└── output/                         # [OUTPUT] ②·③로 흘러가는 산출물
    ...
    └── app-repo-scaffold/
        └── .claude/                # ③ app_repo에 투입될 하네스 자산 골격
                                    # (skills/.claude/hooks/.claude/subagents/.claude/rules)
```
