...
Gate A 통과 조건 (AND):
  ...
  5. PI 승인 확인 (--pi-approved 플래그 또는 환경변수 GATE_A_PI_APPROVED=1)
"""
...
def check_pi_approved(args: list[str]) -> bool:
    if "--pi-approved" in args or os.environ.get("GATE_A_PI_APPROVED") == "1":
        ok("PI 승인 확인됨")
        return True
    block("PI 승인 플래그 없음 (--pi-approved 또는 GATE_A_PI_APPROVED=1 필요)")
    return False
