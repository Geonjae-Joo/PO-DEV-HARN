...
7. **DP 미리보기 렌더(결정론적)** — 검증 통과 후 엔진으로 DP 골격을 렌더해 PO가 본다:
   ...
   - **시각 충실도(ADR-002 D8)**: `foundation/design-system/`에 `ds-compiled.css`·`ds-fixtures.json`가 있으면(ds-bootstrap Phase 9 / `build_ds_assets.mjs` 산출) fixture가 있는 컴포넌트는 **실제 DS 모양**으로, 없으면 라벨박스 와이어프레임으로 렌더된다. 디자인이 비어 보이면 자산이 없는 것 — 먼저 `build_ds_assets.mjs`를 실행한다.
