...
- `render/` — **결정론적 렌더 엔진**(ADR-002): YAML screen/design page → 바이트 동일 HTML, DS 카탈로그, DP→SCR 인스턴스화, `layout_hash`/`render_hash` 핀 계산. ①②③이 동일 엔진을 공유한다.
...
| **패키지** | `packages/plugin-prerequisite` | `packages/po-dev-chat` | `packages/plugin-ai-web-dev` |
| **상세 문서** | [README](packages/plugin-prerequisite/README.md) | [README](packages/po-dev-chat/README.md) | [README](packages/plugin-ai-web-dev/README.md) |

> ①③은 `marketplace.json`에 등록된 Claude Code 플러그인이다(`/plugin marketplace add .`). ②는 Agent SDK 챗봇으로 빌드 예정이라 소스(skill·hook·rule)만 보존하며 마켓플레이스에 등록하지 않는다 — 빌드 방법은 [`docs/CHATBOT-DEV-GUIDE.md`](docs/CHATBOT-DEV-GUIDE.md).
...
**워크플로우**: 기존 DS를 `foundation/design-system/ds-source/`에 투입 → `ds-allowlist.md` 작성(허용집합 원본) → `design-page-builder` 스킬로 캔버스 모델(locked/editable·grid·breakpoints) 기반 페이지 템플릿(DP-*) 생성 → 렌더 엔진으로 **DS 카탈로그·DP 미리보기** 생성 → 불변 rules + 프로젝트 결정(tech-stack·ops-stack)·SPEC-000·SPEC-OPS-000 명세 확정 → 빈 `app_repo` 스캐폴드.

**핵심 자산**: `design-page-builder` 스킬(+전용 `design-page-lint.py`), 저장 이벤트 훅 `ds-guide-validate.py`, 공유 렌더 엔진(`harness-core/render/render_designpage.py`·`render_catalog.py`), 불변 규칙 3종(constitution/spine-ids/ds-closure, `harness-core/rules/`), 프로젝트 결정 2종(tech-stack/ops-stack, `foundation/decisions/`).
...
> `projects/` 의 실제 데이터(예: `projects/todo`)는 이 문서 범위 밖이다. 새 프로젝트는 `projects/<customer-id>/`로 증식하며, 세 레이어는 같은 폴더를 참조한다.
