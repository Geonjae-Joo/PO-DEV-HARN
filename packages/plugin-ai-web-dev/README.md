...
| `layout-hash-guard.py` | Phase α 진입 | 각 pack `screens[].yaml_ref`를 ②와 동일 엔진(`harness-core/render/pins`)으로 재렌더해 `pinned_contract.layout_hash`와 비교(불일치→빌드 차단). `render_hash`는 warn. (ADR-002 D5) |
