<!-- rule: ③ AI-WEB-DEV — Gate B (개발자 소유) -->

# Gate B Checklist

Phase β에서 `/speckit.tasks` 완료 후, **구현(`/speckit.implement`) 시작 전**에 통과해야 하는 관문.
개발자가 소유한다. PO는 acceptance에 대한 **비차단 소프트 리뷰**만 한다.
...
- **Phase α 진입 시 `layout-hash-guard.py` 통과 필수** — 각 화면을 ②와 동일 엔진으로 재렌더해
  `pinned_contract.layout_hash`와 일치해야 한다. 불일치(②확정 위치 변경)면 빌드 차단(ADR-002 §5).
...
- PO는 acceptance가 의도와 맞는지 확인할 수 있으나, 이 리뷰는 **Gate B를 막지 못한다**.
