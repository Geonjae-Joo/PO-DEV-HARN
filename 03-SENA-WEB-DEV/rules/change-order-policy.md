<!-- rule: 03-SENA-WEB-DEV — Change Order 정책 (③ → ② 인계) -->
...
2. **Freeze** — 구현 중인 화면은 소프트 프리즈. PI 편집은 즉시 반영되지 않고 **change-order 큐**에 누적된다.
3. **Change Order 발행** — PI 재확정 시, 스파인 ID 단위 diff + **blast radius**(영향 화면·팩 범위)를 계산해 변경 지시서를 만든다.
4. **개발자 판정** — 셋 중 하나:
