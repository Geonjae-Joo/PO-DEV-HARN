...
6. **검증(필수)** — 저장 직후 Bash로 전용 린터를 직접 호출한다. **프로젝트 루트(`projects/<id>/`)에서 실행**해야 `foundation/` 경로가 맞는다(다른 cwd면 `--root <project>` 또는 `PROJECT_ROOT` 지정):

   ```bash
   python "${CLAUDE_PLUGIN_ROOT}/skills/design-page-builder/scripts/design-page-lint.py"
   ```
