...
→ **산출물 D2**: `.claude/hooks/tdd-gate.py` + `.claude/hooks/hooks.json`(PreToolUse/commit 시점).
...
| **D2** | `.claude/hooks/tdd-gate.py` + `.claude/hooks/hooks.json` | D3 | 테스트 없는 더미 커밋 → 거부됨 / `[SCAFFOLD]` → 통과 (픽스처로 검증) |
...
- 본 저장소 내부: `03-AI-WEB-DEV/.claude/rules/tdd-policy.md`, `.claude/skills/speckit-{specify,plan,tasks}/SKILL.md`, `.claude/subagents/{test-author,bl-analyst,code-reviewer}.md`, `02-PO-DEV-CHAT/.claude/skills/spec-generator/spec-pack-schema.md`, `.specify/workflows/speckit/workflow.yml`
