...
description: >
  ...
  PI가 "이 버튼은 뭐 해요?", "동작 정의", "기능 정의"를 언급하거나
...
- Q-ERROR 답변 후 케이스를 분리해 구조화. PI가 케이스를 구분하지 않으면 `default`만.
...
- `outcome.type` 판단 기준: `rules/question-bank.md` outcome.type별 섹션 참조.
...
- **acceptance는 PI 관점으로 작성**: 기술 구현이 아닌 "사용자가 무엇을 경험하는가" 중심.
