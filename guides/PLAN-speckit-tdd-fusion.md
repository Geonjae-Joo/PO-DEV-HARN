...
- **입력 계약**: `02-PO-DEV-CHAT` → `PACK-*/spec-pack.yaml` (screen model 추출물, 코드 없음)
...
`.claude/rules/tdd-policy.md`·`code-reviewer`가 `tdd-gate.py`를 전제하지만 `03-AI-WEB-DEV/.claude/hooks/` 폴더 자체가 없다. **테스트 없는 구현을 막는 강제 장치 부재** → 정책이 선언으로만 존재.
...
- 본 저장소 내부: `03-AI-WEB-DEV/.claude/rules/tdd-policy.md`, `.claude/skills/speckit-{specify,plan,tasks}/SKILL.md`, `.claude/agents/{test-author,bl-analyst,code-reviewer}.md`, `02-PO-DEV-CHAT/.claude/skills/spec-generator/spec-pack-schema.md`, `.specify/workflows/speckit/workflow.yml`
