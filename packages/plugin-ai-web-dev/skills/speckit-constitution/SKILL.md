...
2. **프로젝트별 기술 스택** — 프론트·백엔드 프레임워크와 2계층 테스트 러너를 **`foundation/decisions/tech-stack.md`**(프로젝트 결정, 규칙 아님)에 정의한다. **하니스는 특정 스택을 가정하지 않는다 — 프론트엔드도 여기서 결정**한다(React/Vue/Svelte…, Spring/Nest/FastAPI… 모두 가능). 이어서 `.specify/memory/constitution.md`의 §Technology Stack 표와 `HARNESS_TEST_CMD`(tdd-gate용)를 그 값으로 갱신한다.
