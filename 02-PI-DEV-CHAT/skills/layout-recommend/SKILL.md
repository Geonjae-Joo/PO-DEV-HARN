...
description: >
  Stage 1 스킬. PI가 나열한 컴포넌트를 DS에 매핑해 screen model YAML 초안을 생성하고,
  model 변경 시마다 design token 기반 HTML 렌더를 자동 생성한다.
  PI가 "레이아웃", "화면 구성", "컴포넌트 배치"를 언급하거나 새 화면을 시작할 때 사용.
when_to_use: >
...
**입력:** PI의 자연어 컴포넌트 나열 + `foundation/design-system/design-guide.md` + `foundation/design-pages/`

**프로세스:**

1. PI 발화에서 컴포넌트 의도 추출
2. `design-guide.md` 허용 목록에서 가장 적합한 DS 컴포넌트 매핑
   - 1:1 대응이 명확하면 바로 매핑
   - 애매하면 후보 2~3개 제시, PI 선택
...
5. 초안 PI에게 제시 → patch 단위 수정 요청 시 반영
