...
>
> ⚠️ 이 가이드는 **프론트엔드 스택을 React + shadcn/ui로 정한 경우**의 예시다. 스택은 `.claude/rules/tech-stack.md`에서 프로젝트별로 정하며 고정이 아니다. 다른 프레임워크/DS를 쓰면 그에 맞는 준비 절차로 대체하되, "DS를 `design-guide.md` 허용 집합으로 등록한다"는 원칙은 동일하게 유지한다.
>
...
- 이 예시가 전제하는 스택(`.claude/rules/tech-stack.md`에서 확정, 고정값 아님): React 18 + Vite + TypeScript + Tailwind CSS + shadcn/ui.
...
> 원칙(`.claude/rules/tech-stack.md`): **색·간격·타이포는 토큰만 참조**한다. 컴포넌트에 hex/px를 하드코딩하지 않는다.
...
python .claude/hooks/ds-guide-validate.py output/foundation/design-system/design-guide.md
```
...
세 레이어 모두 **`design-guide.md`에 등록된 컴포넌트만** 쓸 수 있다 — 이것이 DS 폐쇄(`.claude/rules/ds-closure.md`)다.

---

## 8. 새 컴포넌트가 더 필요해질 때

PO가 허용 집합에 없는 컴포넌트를 원하면, 임의로 추가하지 않고 절차를 밟는다(`.claude/rules/ds-closure.md`).
...
# 4) 검증
python .claude/hooks/ds-guide-validate.py output/foundation/design-system/design-guide.md
python .claude/skills/design-page-builder/scripts/design-page-lint.py   # 기존 DP 영향 확인
```
